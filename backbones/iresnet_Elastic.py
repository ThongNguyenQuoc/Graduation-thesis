...
from utils.countFLOPS_Elastic import _calc_width, count_model_flops
...
class IResNetThongSoi(nn.Module):
    fc_scale = 7 * 7
    def __init__(self,
                 block, layers, dropout=0, num_features=512, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None, use_se=False):
        super(IResNetThongSoi, self).__init__()
        self.inplanes = 64
        self.dilation = 1
        self.use_se=use_se
        if replace_stride_with_dilation is None:
...
def _iresnet(arch, block, layers, pretrained, progress, **kwargs):
    model = IResNetThongSoi(block, layers, **kwargs)
    if pretrained:
        raise ValueError()
    return model

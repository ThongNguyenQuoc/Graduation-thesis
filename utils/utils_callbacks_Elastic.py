...
from eval import verification_Elastic
from utils.utils_logging_Elastic import AverageMeter
...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            acc1, std1, acc2, std2, xnorm, embeddings_list = verification_Elastic.test(
    ...
    def init_dataset(self, val_targets, data_dir, image_size):
        for name in val_targets:
            ...
            if os.path.exists(path):
                data_set = verification_Elastic.load_bin(path, image_size)
